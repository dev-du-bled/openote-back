from fastapi import APIRouter, Header, HTTPException, status, UploadFile, Request
...
async def upload_logo_endp(file: UploadFile, rq:Request, Authorization: str = Header(...)):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as c:
        ...
        c.execute("""UPDATE "user" SET profile_picture=%s WHERE id=%s;""", (url, userid))
        conn.commit()
